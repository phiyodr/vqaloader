...
class GQADataset(Dataset):
    ...
    def __init__(self, split, balanced=True, data_path="",
                 image_transforms=None, question_transforms=None, tokenize=clip_tokenize,
        ...
        if not balanced and split == "train":
            ...
            path = os.path.expanduser(os.path.join(data_path, "gqa", self.file_name))
            if verbose:
    ...
    def __getitem__(self, index):
        ...
        image_path = os.path.expanduser(os.path.join(self.data_path, "gqa/images", f"{image_id}.jpg"))        
        print(image_path)
...
class TextVQADataset(Dataset):
    ...
    def __init__(self, split, version="0.5.1", data_path="",
        ...
        self.file_name = f"TextVQA/{split}/TextVQA_{version}_{cleaner(split)}.json"
    ...
    def __getitem__(self, index):
        ...
        # Load and transform image
        image_path = os.path.expanduser(os.path.join(self.data_path, f"TextVQA/{self.IMAGE_PATH[self.split]}/{image_id}.jpg"))        
...
if __name__ == '__main__':
    ...
    print("===GQADataset===")
    dataset = GQADataset(split="train", balanced=True, data_path="~/Data/", testing=False)
    print(dataset[0])

    print("===TextVQADataset===")
    dataset = TextVQADataset(split="train",  data_path="~/Data/", testing=False)
    print(dataset[0])